...
class ArticleService:
    ...
    def calPopularity(self,article):
        return float('%.2f' % (article.upvoteNum // max((article.upvoteNum + article.downvoteNum), 1) * (article.accessNum * 0.3 + article.commentNum * 0.7)))
...
class CommentService:
    ...
    def search(self, content):
        ...
        comments.extend(Comment.query.filter(Comment.email.like('%%%s%%' % content)).all())

        rec = []
        result = []
        for comment in comments:  # remove the repeat articles
